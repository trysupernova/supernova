...
func GetBaseUrlWebApp() string {
	return os.Getenv("BASE_URL_WEB_APP")
}
...
func (u User) generateAndSendPasswordReset() error {
	// send the token in the email
	// when the user clicks the link, we can verify the token and allow them to reset their password
	...
	if os.Getenv("ENVIRONMENT") == "dev" {
		println("reset token generated: " + token)
	}
	...
	compiledEmail, err := email.CompileEmailForgotPassword(fmt.Sprintf("%s/forgot-password/verify?token=%s", GetBaseUrlWebApp(), token))
	if err != nil {
		return err
	}
