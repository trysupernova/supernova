...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if user.checkPassword(userBody.Password) {
		...
		customHTTP.NewErrorResponse(w, http.StatusUnauthorized, errors.New("Password incorrect"))
		return
	}
}
...
func ChangePasswordHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := db.DB.First(&user, requestingUserId).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			customHTTP.NewErrorResponse(w, http.StatusBadRequest, errors.New("User not found"))
			return
		}
		customHTTP.NewErrorResponse(w, http.StatusBadRequest, err)
		return
